...
def build_cnn():
    model = Sequential()
    #model.add(Conv2D(64, (3, 3), activation='relu', input_shape=(h, w, dp))) # 200x150x3 => 198x148x32
...
#tensorboard = TensorBoard(log_dir='logs/',  write_images=False,write_graph=False, write_grads=True, histogram_freq=1)
model = build_cnn()

print('Training')

#model.fit(X_train, Y_train,  batch_size=32, epochs=10, verbose=1, validation_data=(X_test, Y_test), callbacks=[tensorboard])
